...
class DCepNet(nn.Module):

    def __init__(self, num_classes=1000,  pretext_classes=4):
        super(DCepNet, self).__init__()

        incep1 = inception_v3_mod(pretrained=True)
          #incep1.fc = nn.Identity()
          #incep1.AuxLogits.fc = nn.Linear(768, num_classes)
          #incep1.AuxLogits.fc.apply(init_weights)
        incep2 = inception_v3_mod(pretrained=True)
          #incep2.fc = nn.Identity()
          #incep2.AuxLogits.fc = nn.Linear(768, num_classes)
          #incep2.AuxLogits.fc.apply(init_weights)
        # The output of both "incomplete" resnet18 is 512 x 7 x 7 (C x H x W)
        # The concatenation of results is a tensor 1024 x 7 x 7

        self.model_RGB = incep1#nn.Sequential( *list(resnet1.children())[:-2])
        self.model_DEPTH = incep2#nn.Sequential( *list(resnet2.children())[:-2])
        ...
        self.Pbranch = nn.Sequential(
            ...
            nn.Conv2d(4096,100, 1), # -> 100 x 7 x 7
            nn.BatchNorm2d(100),
            nn.PReLU(100),
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(100, pretext_classes)
        )
        self.Pbranch.apply(init_weights)

        self.MAuxBranch = nn.Sequential(
            nn.AdaptiveAvgPool2d((1,1)),
            nn.Flatten(),
            nn.Linear(768*2, num_classes),
        )
        self.PAuxBranch = nn.Sequential(
            nn.AdaptiveAvgPool2d((1,1)),
            nn.Flatten(),
            nn.Linear(768*2, pretext_classes),
        )
        self.MAuxBranch.apply(init_weights)
        self.PAuxBranch.apply(init_weights)

    def forward(self, x_rgb, x_depth, mode="main", debug=False):
        if mode == "main":
          if self.training:
            x_rgb, x_rgb_aux = self.model_RGB(x_rgb)
            x_depth, x_depth_aux = self.model_DEPTH(x_depth)
            x_aux_cat = torch.cat([x_depth_aux, x_rgb_aux], dim=1)
            x_out_aux = self.MAuxBranch(x_aux_cat)
          else:
          ...
          if self.training:
            ...
            x_depth, x_depth_aux = self.model_DEPTH(x_depth)
            x_aux_cat = torch.cat([x_depth_aux, x_rgb_aux], dim=1)
            x_out_aux = self.PAuxBranch(x_aux_cat)
          else:
            x_rgb = self.model_RGB(x_rgb)
            x_depth = self.model_DEPTH(x_depth)
...
def inception_v3_mod(pretrained=False, progress=True, **kwargs):
...
class Inception3(nn.Module):
    ...
    def __init__(self, aux_logits=True, transform_input=False,
        ...
        if aux_logits:
            self.AuxLogits = inception_aux(768)
...
class InceptionAux(nn.Module):
    ...
    def __init__(self, in_channels, conv_block=None):
        ...
        """
        self.fc_main = nn.Linear(768, num_classes)
        self.fc_main.stddev = 0.001
        self.fc_pretext = nn.Linear(768, pretext_classes)
        self.fc_pretext.stddev = 0.001
        """
    def forward(self, x):
        # N x 768 x 17 x 17
