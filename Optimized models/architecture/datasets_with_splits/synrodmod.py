...
class SynRODMOD(VisionDataset):
    ...
    def __init__(self,
        ...
        with open(synarid_path) as file:
            for line in file:
                path, label = line.rstrip().split(" ")
                label = int(label)
                rgb_image_file_path = os.path.join(root, path.replace("***", "rgb"))
                depth_image_file_path = os.path.join(root, path.replace("***", "depth"))
                if os.path.exists(rgb_image_file_path) and os.path.exists(depth_image_file_path):
                    self.X.append(("ORIGINAL", (rgb_image_file_path, depth_image_file_path)))
                    self.y.append(label)
                    if augment:
                        self.X.append(("AUGMENT", (rgb_image_file_path, depth_image_file_path)))
                        self.y.append(label)
