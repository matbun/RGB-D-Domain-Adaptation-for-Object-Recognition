...
class TransformConfig:
  def __init__(self, resize_shape=256, centercrop_shape=224): #, config_type="imagenet", pretext="rotation"
    ...
    self.standard_transform = transforms.Compose([transforms.Resize((resize_shape, resize_shape)),
    ...
    self.pre_zoom_transform = transforms.Compose([                      #select 256 to have a slight zoom when centercropping to 224  # Zoom to fit the image in 224 x 224
                                              transforms.Resize((resize_shape, resize_shape)),  # or select 224 to not have an initial slight zoom on the image
    ...
    self.synrod_rgb_transform = transforms.Compose([transforms.Resize((resize_shape, resize_shape)),
                                          transforms.CenterCrop(centercrop_shape),
                                          transforms.ToTensor(),
                                          self.normalize_synrod_rgb])
    self.synrod_depth_transform = transforms.Compose([transforms.Resize((resize_shape, resize_shape)),
                                          transforms.CenterCrop(centercrop_shape),
                                          transforms.ToTensor(),
                                          self.normalize_synrod_depth])
    self.rod_rgb_transform = transforms.Compose([transforms.Resize((resize_shape, resize_shape)),
                                          transforms.CenterCrop(centercrop_shape),
                                          transforms.ToTensor(),
                                          self.normalize_rod_rgb])
    self.rod_depth_transform = transforms.Compose([transforms.Resize((resize_shape, resize_shape)),
                                          transforms.CenterCrop(centercrop_shape),
                                          transforms.ToTensor(),
    ...
    self.standard_post_zoom_transform = transforms.Compose([
                                            transforms.Resize((centercrop_shape, centercrop_shape)),
                                            transforms.ToTensor(),
                                            transforms.Normalize(self.imgnet_means, self.imgnet_std)])
